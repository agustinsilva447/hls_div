...
void write_FIFO(UINTPTR baseaddr, u32 *recv_buf, u32 length, u32 *send_buf) {
	volatile u32 i = 0;
	while(!(cbRead(baseaddr, CB_OFIFO_STATUS) & 0x01) && (i < length)) {
		cbWrite(baseaddr, CB_OFIFO_VALUE, recv_buf[i + 2]);
		i++;
		send_buf[1] = i;
	}
    if (cbRead(baseaddr, CB_OFIFO_STATUS) & 0x04) {
    	send_buf[0] = OVERFLOW; // if overflow there was an error
    } else {
    	send_buf[0] = SUCCESS;
    }
}
...
void write_RAM(UINTPTR baseaddr, UINTPTR offset, u32 *recv_buf, u32 length, u32 inc, u32 *send_buf) {
	if (inc == 1) {
		if(logging)
			xil_printf("Writing: %d, %d \n\r", baseaddr + offset, (int *)(recv_buf + 4));
		cbWriteBulk(baseaddr + offset, (int *) (recv_buf + 4), length);
		send_buf[1] = length;
	}else {
		volatile u32 i;
		for (i = 0; i < length; i ++) {
			if(logging)
				xil_printf("Writing: %d, %d, %d \n\r", baseaddr + offset, i * 4 * inc, recv_buf[i + 4]);
			cbWrite(baseaddr + offset, i * inc, recv_buf[i + 4]);
			send_buf[1] = i;
		}
	}
	send_buf[0] = SUCCESS;
...
void read_MEM(UINTPTR baseaddr, u32 *send_buf, u32 length, u32 inc) {
	...
	send_buf[0] = SUCCESS; // success always assumed, unprotected operations are responsibility of the user
}

void write_MEM(UINTPTR baseaddr, u32 *recv_buf, u32 length, u32 inc, u32 *send_buf) {
	if (inc == 1) {
		memmove((UINTPTR *)baseaddr, (int *) (recv_buf + 4), length);
		send_buf[1] = length;
	} else {
		volatile u32 i;
		for (i = 0; i < length; i ++) {
			cbWrite(baseaddr, i * inc, recv_buf[i + 4]);
			send_buf[1] = i;
		}
	}
	send_buf[0] = SUCCESS; // success always assumed, unprotected operations are responsibility of the user
...
int process_command(u32 *recv_buf, int sd) {
	...
	send_buf[0] = FAIL; // always error unless a successful operation
	...
	switch(pack_type){
		...
		case LOG:
			if(recv_buf[1]) {
				...
				send_buf[0] = LOG_ENABLE;
			}else{
				logging = false;
				xil_printf("Logging disabled. \n\r");
				send_buf[0] = LOG_DISABLE;
			}
			write(sd, send_buf, 4);
		default:
			break;
